...
WindowsWatcher = require '../lib/watcher.js'

watcherDLLpath = path.resolve(__dirname, '..', 'bin', 'Watcher.dll')
...
describe 'watcher', ->
	...
	describe '.constructor', ->
		it 'accepts alternative edge', ->
			fakeEdgeObject = {}
			stub = sinon.stub(fakeEdgeObject, "func");
			stubExistsSync = sinon.stub(fs, "existsSync");
			stubExistsSync.withArgs(watcherDLLpath).returns(true);


			expect(new watcher(edge)).to.not.throw(Error)
			expect(stub).to.have.been.calledWith({
				assemblyFile: watcherDLLpath,
				typeName: 'NativeWatcher.Startup',
				methodName: 'Invoke'})

	describe '.watch', ->
		it 'accepts valid string and callback without crashing', ->
			expect(new watcher().watch("C:/", ()->)).to.not.throw(Error)
